        ...
        <div class="speaker-wrap ftco-animate d-flex">
            ...
            <div class="text pl-md-5">
                ...
                <c:if test="${role.id == 2}">
                    ...
                    <span>
                        <button type="button" class="btn btn-success join-club-agree"
                                value="${item.apply.id}" clubId="${item.apply.clubid}" proposerid="${item.apply.proposerid}">同意</button>
                        <button type="button" class="btn btn-danger join-club-deny"
...
<script>
    $().ready(function () {
        $(".join-club-agree").click(function () {
            var obj = $(this).parents('span');
            $.ajax({
                type: "POST",
                url: "${pageContext.request.contextPath}/clubs/join_agree",
        ...
        $(".join-club-deny").click(function () {
            var obj = $(this).parents('span');
            $.ajax({
                type: "POST",
                url: "${pageContext.request.contextPath}/clubs/join_deny",
                data: {'applyId': $(this).attr("value"), 'clubId': $(this).attr("clubId"), 'proposerid': $(this).attr("proposerid")},
                success: function () {
                    obj.text('拒绝')
                }
